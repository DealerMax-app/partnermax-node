...
export interface ClientOptions {
  /**
   * Partner API key issued via /v1/keys/issue. Preferred authentication for server-to-server traffic.
   */
  apiKey?: string | null | undefined;

  /**
   * Same partner API key, passed as `Authorization: Bearer <key>`. Accepted as an alternative to `X-Api-Key`.
   */
   ...
   *
   * Defaults to process.env['PARTNERMAX_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['PARTNERMAX_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Partnermax API.
 */
export class Partnermax {
  apiKey: string | null;
   ...
   */
  constructor({
    baseURL = readEnv('PARTNERMAX_BASE_URL'),
    apiKey = readEnv('PARTNERMAX_API_KEY') ?? null,
    bearerToken = readEnv('PARTNERMAX_BEARER_TOKEN') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    if (baseURL && opts.environment) {
      throw new Errors.PartnermaxError(
    ...
    this.timeout = options.timeout ?? Partnermax.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('PARTNERMAX_LOG'), "process.env['PARTNERMAX_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('PARTNERMAX_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  static Partnermax = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static PartnermaxError = Errors.PartnermaxError;
